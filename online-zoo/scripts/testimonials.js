const testimonialsPosts = [
  {
    name: "Michael John ",
    avatar: "../../images/testimonials/user_icon.svg",
    location: "Local Austria &bull; Today",
    text: `The best online zoo I’ve met. My son delighted very much ljves to watch
        gouillas.
        Online zoo is one jf the ways to
        instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch
        gouillas. Online zoo
        is one jf the ways to instill a love for animals.
        The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the
        ways to
        instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch
        gouillas. Online zoo
        is one jf the ways to instill a love for animals.`,
  },
  {
    name: "Oskar Samborsky",
    avatar: "../../images/testimonials/men1.svg",
    location: "Local Austria &bull; Yesterday",
    text: `Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.

The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
`,
  },
  {
    name: "Fredericka Michelin",
    avatar: "../../images/testimonials/men2.svg",
    location: "Local Austria &bull; Yesterday",
    text: `The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
`,
  },
  {
    name: "Mila Riksha",
    avatar: "../../images/testimonials/men3.svg",
    location: "Local Austria &bull; Yesterday",
    text: `My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
`,
  },
  {
    name: "Michael John ",
    avatar: "../../images/testimonials/user_icon.svg",
    location: "Local Austria &bull; Today",
    text: `The best online zoo I’ve met. My son delighted very much ljves to watch
        gouillas.
        Online zoo is one jf the ways to
        instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch
        gouillas. Online zoo
        is one jf the ways to instill a love for animals.
        The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the
        ways to
        instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch
        gouillas. Online zoo
        is one jf the ways to instill a love for animals.`,
  },
  {
    name: "Oskar Samborsky",
    avatar: "../../images/testimonials/men1.svg",
    location: "Local Austria &bull; Yesterday",
    text: `Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.

The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
`,
  },
  {
    name: "Fredericka Michelin",
    avatar: "../../images/testimonials/men2.svg",
    location: "Local Austria &bull; Yesterday",
    text: `The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
`,
  },
  {
    name: "Mila Riksha",
    avatar: "../../images/testimonials/men3.svg",
    location: "Local Austria &bull; Yesterday",
    text: `My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
`,
  },
  {
    name: "Michael John ",
    avatar: "../../images/testimonials/user_icon.svg",
    location: "Local Austria &bull; Today",
    text: `The best online zoo I’ve met. My son delighted very much ljves to watch
        gouillas.
        Online zoo is one jf the ways to
        instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch
        gouillas. Online zoo
        is one jf the ways to instill a love for animals.
        The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the
        ways to
        instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch
        gouillas. Online zoo
        is one jf the ways to instill a love for animals.`,
  },
  {
    name: "Oskar Samborsky",
    avatar: "../../images/testimonials/men1.svg",
    location: "Local Austria &bull; Yesterday",
    text: `Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.

The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
`,
  },
  {
    name: "Fredericka Michelin",
    avatar: "../../images/testimonials/men2.svg",
    location: "Local Austria &bull; Yesterday",
    text: `The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
`,
  },
  {
    name: "Oskar Samborsky",
    avatar: "../../images/testimonials/men1.svg",
    location: "Local Austria &bull; Yesterday",
    text: `Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas.
The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.

The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.The best online zoo I’ve met. My son delighted very much ljves to watch gouillas. Online zoo is one jf the ways to instill a love for animals.
`,
  },
];

const testimonials = document.querySelector(".testimonials");
const postContainer = testimonials.querySelector(".testimonials__posts");
const inputScroll = testimonials.querySelector(".testimonials__input");
const templatePost = document.querySelector("#template-testimonials").content;
let postsPosition = 0;
let value = 0;
let privValue = inputScroll.value;

function addPost(el) {
  const sliderEl = templatePost
    .querySelector(".testimonials__post")
    .cloneNode(true);

  sliderEl.querySelector(".testimonials__avatar").src = el.avatar;
  sliderEl.querySelector(".testimonials__name").textContent = el.name;
  sliderEl.querySelector(".testimonials__location").textContent = el.location;
  sliderEl.querySelector(".testimonials__text").textContext = el.text;

  return sliderEl
}


function initPosts() {
  for(const post of testimonialsPosts) {
    postContainer.append(addPost(post))
  }
}

function setMax() {
if(screen.width <= 1024) {inputScroll.max = 9}
if (screen.width > 1024) {
  inputScroll.max = 8;
}
inputScroll.value = 0;
value = 0;
privValue = 1;
}
setMax();
initPosts();
setPosts();


window.addEventListener("resize", () => {
   let el = postContainer.querySelectorAll(".testimonials__post");
   postsPosition = 0;
   for (const elem of el) {
     elem.style.transform = `translateX(${postsPosition}px)`;
   }
setPosts();
setMax();
})

inputScroll.oninput = move;
function move() {
  let el = postContainer.querySelectorAll('.testimonials__post')

const move = el[0].clientWidth + 35;
 privValue = inputScroll.value;
 console.log(privValue,value)
if(privValue > value) postsPosition -= move;
if (privValue < value) postsPosition += move;
value = privValue;
  for (const elem of el) {
    elem.style.transform = `translateX(${postsPosition}px)`;
  }
}

function setPosts() {
  if (testimonials.clientWidth < 950) {
    setPostsNone();
  } else {
    setPostVisible();
  }
}


function setPostsNone() {
  let posts = postContainer.querySelectorAll(".testimonials__post");
  for (let i = 3; i < posts.length; i++) {
    posts[i].classList.add("testimonials__post_notvisible");
  }
}
function setPostVisible() {
  let posts = postContainer.querySelectorAll(".testimonials__post");
  for (let i = 0; i < posts.length; i++) {
    posts[i].classList.remove("testimonials__post_notvisible");
  }
}